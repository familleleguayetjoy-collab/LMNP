...
def operations_od_factures(factures, dico, resolver=None, *,
    ...
    `seulement_payees=True` — le cas d'un dossier AVEC banque : une facture qui
    ne se déclare pas payée et qu'on ne retrouve pas au relevé n'a, pour
    l'instant, jamais été réglée. Pas de flux, pas d'écriture : elle reste
    ouverte, on interroge le client, et elle tombera dans l'exercice où elle
    sera payée. Lui passer une écriture créerait une charge que personne n'a
    décaissée.

    Par défaut on les prend toutes, parce que c'est le cas du dossier SANS
    banque : il n'y a pas de relevé pour contredire les pièces, et tout ce qui a
    été réglé l'a été par l'exploitant."""
    ...
    for f in factures:
        ...
        if seulement_payees and not (getattr(f, "payee", False)
                                     or getattr(f, "date_reglement", None)):
            continue                      # aucun règlement identifié -> rien
